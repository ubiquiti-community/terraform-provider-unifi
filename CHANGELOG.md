...
- **`unifi_setting.ips`: stop `suppression_alerts`/`suppression_whitelist` showing a perpetual diff.** These were modeled as a nested field of the `ips` setting, but current controllers store IPS suppression under a separate `ips_suppression` API endpoint, so the `ips` setting response never echoes the configured entries back — every plan showed the whole list as needing to be reapplied. The provider now mirrors the configured value into state instead of the (always empty) server response, and surfaces a warning that it cannot verify the suppression rules were actually applied on the controller — full read/write support requires go-unifi to add the `ips_suppression` endpoint (#381)

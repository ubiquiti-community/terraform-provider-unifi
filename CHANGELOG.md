...
- **`unifi_firewall_policy`: fix `inconsistent result after apply` on `source`/`destination` `matching_target_type` when updating a policy (e.g. changing `action`).** This field is firmware-derived and the controller may re-derive it during the update (e.g. `""` → `"SPECIFIC"` when a specific port is set), which the provider cannot predict. The update path now re-asserts the planned value on the post-apply state, leaving the next refresh to reconcile it with the controller (#324)

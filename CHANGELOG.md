...
- **`unifi_device`: stop a partially-declared `radio_table` block sending `antenna_gain`/`antenna_id`/`ht` as a literal `0`.** A block that only sets some sub-fields (e.g. `radio`/`channel`/`ht`/`tx_power_mode`, as in #427's repro) leaves the rest `Unknown` rather than `Null`, since they are `Optional + Computed` with no per-field plan modifier to resolve them from state. `frameworkToRadioTable` converted `Unknown` the same as a real configured value via `ValueInt64Pointer`, which returns a pointer to the Go zero value for `Unknown` — so an unset `antenna_id`/`antenna_gain` (which have no "enabled" gate of their own) went out as `0`, and the controller rejects it with `api.err.InvalidPayload` (400), the same class of failure `min_rssi`/`sens_level`/`assisted_roaming_rssi`/`maxsta` already had guarded via `sanitizeRadioForUpdate`'s enabled-flag/range check (#378). `Unknown` sub-fields are now treated the same as `Null` — omitted from the PUT rather than zero-filled (#427)

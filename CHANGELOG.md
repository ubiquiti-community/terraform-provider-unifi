...
- **`unifi_network`: warn when `dhcp_guarding.servers` is configured on a `corporate`/`guest` network.** go-unifi only marshals `dhcpd_ip_1..3` onto the wire for `purpose = "vlan-only"` networks (go-unifi#68); for `corporate`/`guest` purposes those fields are silently dropped before the request reaches the controller, so `terraform apply` reports success with `dhcp_guarding.servers` set in state even though the controller never receives them. `ModifyPlan` now emits a warning on `dhcp_guarding.servers` for those purposes, pointing at go-unifi#68 as the tracking issue, until the upstream client is fixed (#419)
